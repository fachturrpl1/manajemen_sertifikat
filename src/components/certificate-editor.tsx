            ...
            <button
              ...
              onClick={async () => {
                try {
                  ...
                  if (!opened) setMessage('PDF dibuka/diunduh. Jika tidak muncul, cek blocker pop-up.')
                  // Simpan ke tabel (best-effort)
                  try {
                    const pdfBlob = pdf.output('blob')
                    ...
                    await supabase.from('certificate_files').insert(payload)
                  } catch {}
                } catch (err: unknown) {
